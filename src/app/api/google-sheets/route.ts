...
function extractSurahName(videoTitle: string): string {
  ...
  surahName = surahName.replace(/Word-by-Word Meaning & Translation/g, 'Surah Al-Fatiha');
...
async function fetchGoogleSheetsData(): Promise<ProcessedData[]> {
  ...
  try {
    ...
    if (csvText.includes('<!DOCTYPE html>') || csvText.includes('<html>') || csvText.includes('Sign in')) {
      throw new Error('Google Sheet requires authentication. Please share sheet with "Anyone with the link" permission.');
    }
    ...
    if (validLines.length < 2) {
      throw new Error('Not enough data in sheet - need at least headers + 1 data row');
    }
    ...
    const getColumnIndex = (colName: string) => {
      ...
      if (index === undefined) {
        console.log(`Warning: Column '${colName}' not found in headers. Available:`, Array.from(columnMap.keys()).join(', '));
      }
      return index;
    };

    const dateIndex = getColumnIndex('Date');
    const videoTitleIndex = getColumnIndex('Video Title');
    const subscribersIndex = getColumnIndex('Subscribers');
    const viewsIndex = getColumnIndex('Views');
    const likesIndex = getColumnIndex('Likes');
    const commentsIndex = getColumnIndex('Comments');
    const gainedViewsIndex = getColumnIndex('Gained Views');
    const gainedLikesIndex = getColumnIndex('Gained Likes');
    const gainedCommentsIndex = getColumnIndex('Gained Comments');

    console.log('Column indices:', {
      dateIndex, videoTitleIndex, subscribersIndex,
      viewsIndex, likesIndex, commentsIndex,
      gainedViewsIndex, gainedLikesIndex, gainedCommentsIndex
    });
    ...
    for (let i = 0; i < dataLines.length; i++) {
      ...
      const videoTitle = videoTitleIndex >= 0 ? cleanCell(row[videoTitleIndex]) : '';
      ...
      const subscribers = subscribersIndex >= 0 ? Number(cleanCell(row[subscribersIndex])) || 0 : 0;
      const views = viewsIndex >= 0 ? Number(cleanCell(row[viewsIndex])) || 0 : 0;
      const likes = likesIndex >= 0 ? Number(cleanCell(row[likesIndex])) || 0 : 0;
      const comments = commentsIndex >= 0 ? Number(cleanCell(row[commentsIndex])) || 0 : 0;

      // Read Gained values directly from columns
      const gainedViews = gainedViewsIndex >= 0 ? Number(cleanCell(row[gainedViewsIndex])) || 0 : 0;
      const gainedLikes = gainedLikesIndex >= 0 ? Number(cleanCell(row[gainedLikesIndex])) || 0 : 0;
      const gainedComments = gainedCommentsIndex >= 0 ? Number(cleanCell(row[gainedCommentsIndex])) || 0 : 0;

      // Calculate gained subscribers from previous row
      const gainedSubscribers = processedData.length > 0
        ? Math.max(0, subscribers - processedData[processedData.length - 1].subscribers)
        : 0;
...
function groupDataByVideoTitle(data: ProcessedData[]): VideoSummary[] {
  ...
  return Array.from(grouped.values()).sort((a, b) =>
    b.totalGainedViews - a.totalGainedViews
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalGainedComments = data.reduce((sum, d) => sum + d.gainedComments, 0);
    const totalGainedLikes = data.reduce((sum, d) => sum + d.gainedLikes, 0);

    const likeViewRatio = totalGainedViews > 0
      ? (totalGainedLikes / totalGainedViews) * 100
      : 0;

    // Get unique Surah names for filter
    const uniqueSurahNames = Array.from(new Set(data.map(d => d.surahName))).sort();

    // Get unique video titles for filter
    const uniqueVideoTitlesArray = Array.from(new Set(data.map(d => d.videoTitle))).sort();
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('HTTP 403') || error.message.includes('HTTP 401')) {
        ...
        errorDetails = 'The Google Sheet is not publicly accessible. Please share sheet with "Anyone with the link" permission.';
      } else if (error.message.includes('HTTP 404')) {
        errorMessage = 'Google Sheet not found';
        errorDetails = 'The specified spreadsheet ID could not be found. Please check the URL.';
      } else if (error.message.includes('Not enough data')) {
        errorMessage = 'Insufficient data';
        errorDetails = 'The Google Sheet does not contain enough data. Please ensure it has headers and at least one row of data.';
      }
    }
